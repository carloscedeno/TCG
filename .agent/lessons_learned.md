...
### 159. Invariantes de Esquema de Base de Datos vs RPCs Dinámicos (Mayo 2026)
- **Problema**: Violación inmediata de restricción `NOT NULL` en la columna `product_name` de `order_items` al intentar finalizar una compra en producción.
- **Causa Raíz**: Durante la refactorización de funciones atómicas en PL/pgSQL (ej. `create_order_atomic`), las sentencias de inserción omitieron mapear todas las columnas requeridas por el esquema de base de datos.
- **Solución**: Al diseñar o actualizar procedimientos almacenados de órdenes, recuperar de forma explícita todos los metadatos descriptivos del catálogo (`name`, `printing_id`, `finish`, `set_code`) con caídas de respaldo robustas (`COALESCE`) hacia el payload de entrada.
- **Lección**: Nunca asumir que las columnas descriptivas de un ítem de orden son opcionales. En un sistema e-commerce resiliente, congelar los metadatos al momento de la transacción es un requerimiento innegociable de auditoría e inmutabilidad.

### 155. El Anti-patrÃ³n del "Cero Fantasma" en React (Mayo 2026)
- **Problema**: ApariciÃ³n de dÃ­gitos "0" literales en la UI, especialmente en esquinas de tarjetas o grids, sin explicaciÃ³n aparente.
- **Causa RaÃ­z**: Uso de cortocircuito lÃ³gico `&&` con variables numÃ©ricas en JSX (ej: `{stock && <Badge />}`). En JavaScript, `0 && anything` retorna `0`. React renderiza el nÃºmero `0` como un nodo de texto vÃ¡lido, a diferencia de `false`, `null` o `undefined`.
- **SoluciÃ³n**:
    1. **ConversiÃ³n ExplÃ­cita**: Usar `!!variable && <Component />` para forzar a booleano.
    2. **ComparaciÃ³n ExplÃ­cita**: Usar `variable > 0 && <Component />` si el objetivo es mostrar algo solo cuando hay valor.
    3. **Ternario**: Usar `{variable ? <Component /> : null}`.
- **LecciÃ³n**: NUNCA usar variables numÃ©ricas directamente con `&&` en JSX si `0` es un valor posible. Se debe endurecer el tipado y la evaluaciÃ³n lÃ³gica en componentes crÃ­ticos como `Card.tsx` para mantener la limpieza visual de la marca.

### 156. ValidaciÃ³n de Precios Cero en Importaciones (Mayo 2026)
- **Problema**: La carga masiva rechazaba filas con precio "0.00" indicando "Faltan campos obligatorios".
- **Causa RaÃ­z**: Uso de validaciÃ³n `if (!price)` en la Edge Function. En JavaScript, `0` es falsy, por lo que el sistema interpretaba un precio de cero como un campo vacÃ­o.
- **SoluciÃ³n**: Cambiar la validaciÃ³n a `if (isNaN(price))` para permitir explÃ­citamente el valor `0` mientras se siguen rechazando valores no numÃ©ricos o nulos.
- **LecciÃ³n**: Al validar datos financieros, siempre permitir el `0` explÃ­citamente si tiene sentido para el negocio (promos, regalos, placeholders).

### 158. TypeScript Build Strictness in CI (Mayo 2026)
- **Problema**: Despliegues fallidos en producciÃ³n debido a variables e imports no utilizados (`TS6133`).
- **Causa RaÃ­z**: El compilador en entornos de CI (como GitHub Actions o Cloudflare Pages) suele estar configurado con reglas mÃ¡s estrictas que el entorno de desarrollo local.
- **SoluciÃ³n**: Eliminar rigurosamente imports y variables no utilizadas antes de cada push. El uso de herramientas de linting local es mandatorio.
- **LecciÃ³n**: No confiar solo en que "funciona en local"; el build de producciÃ³n es el juez final.

### 159. SPA Navigation Scroll Behavior (Mayo 2026)
- **Problema**: Al navegar a una nueva pÃ¡gina (ej: del CatÃ¡logo al Detalle de Producto), el scroll se mantenÃ­a en la posiciÃ³n anterior, forzando al usuario a "subir" manualmente para ver el contenido.
- **Causa RaÃ­z**: Los navegadores no resetean el scroll automÃ¡ticamente en aplicaciones Single Page (React) a menos que se implemente explÃ­citamente.
- **SoluciÃ³n**: Crear un componente global `ScrollToTop` que escuche cambios en `pathname` y ejecute `window.scrollTo(0, 0)`.
- **LecciÃ³n**: La gestiÃ³n del scroll es una parte fundamental de la experiencia de usuario (UX) en aplicaciones modernas de TCG con listados largos.
...
- **Causa Raíz**:
    1. El frontend realizaba cálculos de precios y mapeos complejos que no siempre coincidían con la lógica del backend (RPC get_user_cart).
    2. La lógica de búsqueda de detalles por UUID intentaba cargar accesorios desde el endpoint de cartas de la Edge Function, causando 404s.
- **Solución**:
    1. **Confianza en el RPC**: Refactorizar etchCart para usar directamente la respuesta plana del RPC unificado, eliminando lógica redundante en el cliente.
    2. **Paridad de Descuentos**: Replicar la lógica de cálculo de ofertas (discount_percentage + discount_until) en el procesamiento del carrito de invitados (pi.ts).
    3. **Priorización de Inventario Local**: Ajustar etchCardDetails para buscar siempre en la tabla ccessories ANTES de recurrir a la Edge Function si el ID es un UUID.
- **Lección**: Centralizar la lógica de precios en la base de datos (RPCs) y asegurar que el frontend sea un reflejo fiel de esos datos. Los accesorios (inventario local) siempre deben tener prioridad de resolución sobre el catálogo global (Cartas) para evitar fallos de ruta.
...
### 157. Remediación de Carga Masiva y Gaps de Catálogo (Mayo 2026)
- **Problema**: Fallos intermitentes en la carga masiva de inventario para sets nuevos o tokens (ej: TM3C, TECC).
- **Causa Raíz**: El sistema asume que el catálogo maestro (`sets`, `card_printings`) está siempre completo. Las importaciones fallan por violaciones de FK si el set o la impresión específica no existen previamente. Además, el uso de `ON CONFLICT` en `products` puede fallar si no se especifica el nombre de la restricción única (`products_printing_id_condition_finish_key`) en entornos con múltiples índices o columnas anulables.
- **Solución**:
    1. **Sincronización Proactiva**: Implementar un paso de "pre-vuelo" que verifique la existencia de IDs de Scryfall y cree los registros faltantes en `sets` y `card_printings` antes de insertar en `products`.
    2. **Endurecimiento de SQL**: Usar `ON CONFLICT ON CONSTRAINT products_printing_id_condition_finish_key` para garantizar una resolución de conflictos atómica y sin ambigüedades.
    3. **Mapeo de Precios**: Permitir precios `0.0` (Free/Token) explícitamente sin que el script de sincronización los descarte como nulos.
- **Lección**: Un importador masivo robusto debe ser capaz de auto-remediar el catálogo maestro usando fuentes externas (Scryfall) si detecta que la base de datos local está desactualizada.
...
### N. Lógica de Descuentos, Fechas NULL y Funciones Duplicadas (PostgREST 300) — 2026-05-22
- **Problema**: El descuento se calculaba en un trigger (sobrescribiendo el precio base en la DB), y al pasar el cálculo al frontend, los descuentos permanentes (sin fecha) dejaron de aplicar porque el frontend y los RPC requerían estrictamente una fecha de fin (discount_end_date). Además, al borrar funciones en Supabase con firmas antiguas, fallaban devolviendo 300 Multiple Choices por haber duplicados (unction overloading).
...
### 6. Filtrado OTHERS en Funciones SQL — 2026-05-26
- **Problema:** La pestaña 'OTROS' en el catálogo mostraba todos los productos (Magic, Digimon, etc.) en lugar de solo los productos sin juego.
...
- **Problema:** Un error PGRST203 ('Could not choose the best candidate function') rompi� la tienda de Producci�n porque hab�a m�ltiples firmas para get_products_filtered.
- **Causa Ra�z:** Al crear una migraci�n SQL, se modific� accidentalmente el orden de los par�metros booleanos. PostgreSQL cre� una funci�n sobrecargada (overloaded) en vez de reemplazar la existente. Adem�s, Cloudflare Pages no ejecuta estas migraciones autom�ticamente, creando discrepancia entre el repositorio y la DB remota.
- **Soluci�n:** Ejecutar expl�citamente DROP FUNCTION de la firma defectuosa y CREATE OR REPLACE con los par�metros en orden estricto, aplicando el SQL manualmente a las BDs de DEV y PROD.
- **Regla Derivada:** LEY 34 (Respetar orden de argumentos en CREATE OR REPLACE FUNCTION) y LEY 35 (Sincronizaci�n manual en Cloudflare).


### 179. Bug en Filtro Cruzado de Categor�as (category vs category_code) � 2026-06-02
- **Problema:** Al seleccionar una categor�a desde la barra lateral (ej. 'Otros', 'Accesorios'), la API devolv�a 'SIN RESULTADOS'.
- **Causa Ra�z:** El frontend asignaba err�neamente la etiqueta de UI de texto libre ('Otros') al par�metro `category_code` en lugar de a `category`. Como `category_code` espera valores de tipo ENUM ('OTHER'), la b�squeda fallaba de manera silenciosa.
- **Soluci�n:** Reasignar el valor pasado por URL a la propiedad `category` dentro del m�todo `fetchAccessories` en `Home.tsx`.
- **Regla Derivada:** Ley 36: DIFERENCIACI�N DE CATEGORY_CODE VS CATEGORY.


### 180. Construcci�n Correcta de URL para Edge Functions - 2026-06-04
...
- **Causa Ra�z:** El frontend intentaba consumir la Edge Function omitiendo el prefijo /api o /tcg-api necesario al concatenar directamente API_BASE y /cards/.... Esto causaba que la llamada HTTP fallara, aunque el sistema se recuperaba consultando la tabla directamente.
- **Soluci�n:** Utilizar la funci�n de utilidad getApiUrl() en etchCardDetails (utils/api.ts) que garantiza la correcta inyecci�n del prefijo, y comprender que los fallbacks a card_printings son mecanismos resilientes esperados.
