...
### 65. Integraci�n de ManaBox y Priorizaci�n de Scryfall ID (Marzo 2026)

- **Problema**: La importaci�n por nombre/set puede fallar en cartas con nombres similares o m�ltiples versiones (promos, showcase).
- **Soluci�n**: Implementar una detecci�n autom�tica de encabezados en el frontend (ManaBox ID, Scryfall ID) y priorizar la b�squeda por scryfall_id en el backend. Esto garantiza una precisi�n del 100% y evita el mapeo manual.
- **Normalizaci�n**: Las condiciones de ManaBox (e.g. 
ear_mint, lightly_played) deben normalizarse en el backend a c�digos internos (NM, LP) para mantener la integridad de la base de datos.
- **UX**: Una pre-visualizaci�n que use los mismos �ndices de mapeo que la l�gica de parseo evita confusiones visuales en el proceso de importaci�n.
