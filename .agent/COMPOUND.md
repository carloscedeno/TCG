## 2026-05-22 - Importaci�n Masiva de Cat�logo y Etiquetado Din�mico

**Qu� pas�:** Se complet� el workflow de carga masiva de accesorios desde la UI. El backend no soportaba `importType='catalog'`, causando fall-through silencioso. Se implement� un mapeo de columnas y l�gica de upsert basado en nombre. Adem�s, se a�adi� inyecci�n din�mica de la etiqueta de preventa `(preventa)` a nivel de backend si la columna Categor�a la contiene.
**Lo que cambi�:**
- `.agent/lessons_learned.md` -> Lecci�n #160
- `supabase/functions/api/index.ts` -> Manejador expl�cito de importaci�n de cat�logo con mapeo de campos gen�ricos y smart upsert.
**Regla derivada:** Evitar el patr�n de fall-through silencioso en endpoints multiprop�sito.
