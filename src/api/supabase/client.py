...
if __name__ == "__main__":
    try:
        ...
        if api.test_connection():
            ...
            cards = api.supabase.table('cards').select('card_id').execute()
            print(f"Cartas únicas (cards): {len(cards.data) if cards and cards.data else 0}")
            
            # Contar impresiones/versiones
            printings = api.supabase.table('card_printings').select('printing_id').execute()
            print(f"Impresiones/versiones (card_printings): {len(printings.data) if printings and printings.data else 0}")
