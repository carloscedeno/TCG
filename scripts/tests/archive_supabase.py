...
accs = db.table('accessories').select('id, name').execute()
to_archive = [a['id'] for a in accs.data if a['name'].strip().lower() not in allowed_names]

print(f"Archiving {len(to_archive)} items from Supabase")
for i in to_archive:
    db.table('accessories').update({'is_active': False}).eq('id', i).execute()
