...
def run_market_sync():
    ...
    supabase_service_key = (os.getenv('SUPABASE_SERVICE_ROLE_KEY') or "").strip().replace('"', '').replace("'", "")
    supabase_anon_key = (os.getenv('SUPABASE_ANON_KEY') or "").strip().replace('"', '').replace("'", "")
