...
**Last Updated**: 2026-04-16 14:30 (Catalog Synchronization Hardening)
**Status**: ✅ Dev Branch Recreation | ✅ Catalog & Inventory Sync | ✅ Quick Add UX Optimized | ✅ Performance Optimization (Batch Fetch) | ✅ Global 'Nuevo' Feature | ✅ Schema Fallback implemented
...
### ✅ Catalog Synchronization Hardening (Compound v50)
- **Schema Constraint Fallback**: Handled PostgreSQL error `42P10` gracefully in PostgREST upserts. The script now dynamically falls back from `game_id,set_code` to `set_code` on unique constraint conflict, adapting intelligently to DB environment disparities mapping GitHub Actions schemas smoothly.
- **OS Text Standard**: Implemented `io.TextIOWrapper` workaround mapping explicit `utf-8` buffers in `sys.stdout` for Windows execution, protecting Python logs with emojis from silent string-rendering crashes during testing.
