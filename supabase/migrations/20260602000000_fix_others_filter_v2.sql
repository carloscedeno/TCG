-- Fix get_accessories_filtered to correctly handle OTHERS filter by including games not in the main list
DO $$ 
DECLARE
  func_record RECORD;
BEGIN
...
BEGIN
    ...
    WITH filtered_data AS (
        ...
        WHERE a.is_active = true
          AND (
              (p_game_code = 'OTHERS' AND (a.game_id IS NULL OR g.game_code NOT IN ('MTG', 'PKM', 'OPC', 'DGM', 'YGO', 'FAB', 'LOR', 'SWU')))
              OR
