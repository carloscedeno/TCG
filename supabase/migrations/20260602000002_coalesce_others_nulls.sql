-- Handle NULL game codes/names in OTHERS filter by using COALESCE
DO $$ 
BEGIN
  -- Recreate functions with COALESCE to prevent NULL NOT IN from evaluating to NULL
END $$;
...
BEGIN
    ...
    WITH filtered_data AS (
        ...
        WHERE a.is_active = true
          AND (
              (p_game_code = 'OTHERS' AND (a.game_id IS NULL OR COALESCE(g.game_code, '') NOT IN ('MTG', 'PKM', 'YGO', 'RFB', 'OPC', 'DGM', 'GND', 'FAB')))
              OR
          ...
          AND (
              ...
              OR a.name ILIKE '%' || TRIM(p_search_query) || '%'
              OR a.description ILIKE '%' || TRIM(p_search_query) || '%'
              OR a.category ILIKE '%' || TRIM(p_search_query) || '%'
          )
