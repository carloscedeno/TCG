-- Update get_products_filtered to include presale filter and handle OTHERS correctly
DO $$ 
DECLARE
  func_record RECORD;
  drop_cmd TEXT;
...
BEGIN
  ...
  WHERE
    ...
    AND (
      v_game_code IS NULL 
      OR (v_game_code = 'OTHERS' AND p.game NOT IN ('MTG', 'PKM', 'OPC', 'DGM', 'YGO', 'FAB', 'Magic', '22', 'Pokemon', 'One Piece', 'Digimon', 'Yu-Gi-Oh', 'Flesh and Blood'))
