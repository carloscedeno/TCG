-- Migration: Restore Accessories Filtering and Add Discounts
-- Date: 2026-05-12
-- Description: Re-adds full filtering capabilities to get_accessories_filtered,
--              adds discount_percentage and discount_until to accessories table,
--              and updates get_products_filtered to support promotional filters.

BEGIN;

-- 1. Add discount columns to accessories table
...
-- 2. Drop existing get_accessories_filtered to cleanly update return signature
DO $$ 
DECLARE
  func_record RECORD;
  drop_cmd TEXT;
...
-- 3. Recreate get_accessories_filtered with complete filtering and full return columns
...
BEGIN
    ...
    WITH filtered_data AS (
        ...
        WHERE a.is_active = true
          AND (p_game_id IS NULL OR a.game_id = p_game_id OR (p_game_code IS NOT NULL AND (g.game_code ILIKE p_game_code OR g.game_name ILIKE p_game_code)))
...
-- 4. Drop existing get_products_filtered to cleanly update return signature with promotional params
DO $$ 
DECLARE
  func_record RECORD;
  drop_cmd TEXT;
...
-- 5. Recreate get_products_filtered with p_only_discount and p_only_presale support
...
GRANT EXECUTE ON FUNCTION public.get_products_filtered(text, text, text[], text[], text[], text[], integer, integer, numeric, numeric, integer, integer, boolean, boolean, boolean, text) TO anon, authenticated;
