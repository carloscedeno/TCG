-- Migration: Fix Cart RPC Synchronization
-- Description: Ensures add_to_cart_v2 and add_accessory_to_cart_v1 select the most recent active cart (ORDER BY updated_at DESC) and update the carts.updated_at timestamp.

BEGIN;

-- 1. Update add_to_cart_v2
...
-- 2. Update add_accessory_to_cart_v1
...
DECLARE
    ...
    v_existing_id uuid;
BEGIN
    -- 1. Get active cart (Priorizando el modificado más recientemente)
    SELECT id INTO v_cart_id FROM public.carts 
    ...
    IF v_existing_id IS NOT NULL THEN
        UPDATE public.cart_items SET quantity = quantity + p_quantity, updated_at = now() WHERE id = v_existing_id;
    ELSE
        INSERT INTO public.cart_items (cart_id, accessory_id, quantity, updated_at)
        VALUES (v_cart_id, p_accessory_id, p_quantity, now());
    END IF;

    -- 3. Actualizar updated_at del carrito para sincronización con get_user_cart
    UPDATE public.carts SET updated_at = now() WHERE id = v_cart_id;
