                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div className="flex flex-col min-w-0 max-w-[200px] md:max-w-md">
                                                        <span className="text-sm font-black text-white italic tracking-tight lowercase truncate">
                                                            {item.name}
                                                            {((item.set_code && ['sos', 'soa', 'soc', 'tsos', 'msh', 'msc', 'mar'].includes(item.set_code.toLowerCase())) || (item.updated_at && new Date().getTime() - new Date(item.updated_at).getTime() < 14 * 24 * 60 * 60 * 1000)) && (
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                {editingPriceId === item.product_id ? (
                                                    ...
                                                    <div className="flex flex-col items-end">
                                                        ...
                                                        >
                                                            {(item.price || 0) === 0 ? 'AUTO [CK]' : `$${(item.discount_percentage && item.discount_percentage > 0 && isDiscountActive(item.discount_end_date) ? (Number(item.price) * (1 - item.discount_percentage / 100.0)) : (Number(item.price) || 0)).toFixed(2)}`}
                                                        </button>
                                                        {item.discount_percentage && item.discount_percentage > 0 && isDiscountActive(item.discount_end_date) && (
                                                            <span className="text-[10px] text-purple-400 font-mono mt-1 bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20">
                                                                Venta: ${(Number(item.price) * (1 - item.discount_percentage / 100) || 0).toFixed(2)}
                                                            </span>
                                                        )}
